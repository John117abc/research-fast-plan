#!/usr/bin/env python3
"""Gate B0-R3 step 2: cross-mechanism pairs + same-mechanism matched controls +
action-level and pair-level one-step closure (from cache; no per-pair search).

usage: python gate_b0/scripts/18_r3_pairs_closure.py
"""
import csv
import json
import os
...
def main():
    ...
    def closure(p, ptype):
        ...
        for u in ACTION_IDS:
            ...
            if vi is not None and vj is not None:
                ...
                valid_only.append(d)
                feas.append(1)
                both_valid, both_inf, one = 1, 0, 0
            elif vi is None and vj is None:
                d1s.append(0.0)
                feas.append(1)
                both_valid, both_inf, one = 0, 1, 0
            else:
                d1s.append(1.0)
                feas.append(0)
                both_valid, both_inf, one = 0, 0, 1
            action_rows.append({"pair_id": "%s__%s" % (i, j), "pair_type": ptype,
                                ...
                                "feasibility_agree": feas[-1], "both_valid": both_valid,
                                "both_infeasible": both_inf, "one_valid": one})
        d1m = float(np.mean(d1s)) if d1s else float("nan")
        return {"pair_id": "%s__%s" % (i, j), "pair_type": ptype, "state_i": i, "state_j": j,
                ...
                "same_param_group": p["same_param_group"], "ego_speed_diff": p["ego_speed_diff"],
                "d0_mean": p["d0_mean"], "d1_mean": round(d1m, 6),
    ...
    # same-mechanism matched controls: nearest d0, distinct param group
    ...
    same_speed = np.array([p["ego_speed_diff"] for p in same])
    D = np.abs(same_d0[None, :] - cross_d0[:, None])
    D[:, same_group == 1] = np.inf
    nearest = np.argmin(D, axis=1)
    matched = []
    for k, p in enumerate(cross):
        m = dict(same[int(nearest[k])])
        m["pair_type"] = "same_matched"
        m["matched_to"] = "%s__%s" % (p["state_i"], p["state_j"])
        matched.append(m)
    for p in matched:
        pair_rows.append(closure(p, "same_matched"))
    ...
    print("cross pairs %d, matched %d, action rows %d" %
          (len(cross), len(matched), len(action_rows)))
